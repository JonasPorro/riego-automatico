...
class _ConfiguracionParametrosState extends State<ConfiguracionParametros> {
    ...
    Future<void> guardarConfiguracion() async {
        ...
        try {
            ...
            FormData formData = FormData.fromMap({'config_data': configData});
            ...
            if (response.statusCode == 200) {
                ...
                setState(() {
                    errorMessage = 'Error al subir el archivo: ${response.statusCode}';
                });
            }
        } catch (e) {
            print('Error: $e');
...
class _HistorialDeMedicionesState extends State<HistorialDeMediciones> {
    ...
    Widget _buildLineChart() {
        ...
        // Filtro  los datos de los últimos 7 días
        final now = DateTime.now();
        final sevenDaysAgo = now.subtract(Duration(days: 7));
        DateFormat dateFormat = DateFormat('MM/dd/yyyy');

        // Construir una lista de datos filtrados por los últimos 7 días
        List<String> filteredLines = lines.where((line) {
            List<String> columns = line.split('|');
            if (columns.length >= 3) {
                DateTime date = dateFormat.parse(columns[0].trim());
                return date.isAfter(sevenDaysAgo) || date.isAtSameMomentAs(sevenDaysAgo);
            }
            return false;
        }).toList();

        // Ordenar las líneas filtradas por fecha y hora
        filteredLines.sort((a, b) {
            List<String> columnsA = a.split('|');
            List<String> columnsB = b.split('|');
            DateTime dateA = dateFormat.parse(columnsA[0].trim());
            DateTime dateB = dateFormat.parse(columnsB[0].trim());
...
Future<String?> leerArchivoRtdo() async {
    try {
        ...
        if (response.statusCode == 200) {
            ...
            final List<String> allLines = response.data.split('\n');
            final DateFormat dateFormat = DateFormat('MM/dd/yyyy');
            final DateTime now = DateTime.now();
            final DateTime today = DateTime(now.year, now.month, now.day);

            // Filtrar las líneas que pertenecen a la fecha actual
            final List<List<String>> todayLines = allLines.where((line) {
                if (line.trim().isEmpty) return false; // Ignorar líneas vacías
                final columns = line.split('|');
                if (columns.length >= 3) {
                    try {
                        DateTime date = dateFormat.parse(columns[0].trim().split(',').first.trim());
                        DateTime parsedDate = DateTime(date.year, date.month, date.day);
                        return parsedDate.isAtSameMomentAs(today);
                    } catch (e) {
                        return false;
                    }
                }
            ...
            // Crear la tabla en formato de texto plano
            final buffer = StringBuffer();
            buffer.writeln('Fecha y hora               | Estación  | Humedad del suelo ');
            //buffer.writeln('-------------|-----------|------');

            for (var line in todayLines.reversed) {
                final dateAndTime = line[0].trim();
                final station = line[1].trim();
                final value = line[2].trim();
                buffer.writeln('$dateAndTime| $station | $value');
            }

            return buffer.toString();
        } else {
            return 'Error al obtener archivo: ${response.statusCode}';
        }
    } catch (e) {
